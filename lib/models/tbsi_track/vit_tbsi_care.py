...
from lib.models.layers.qkv_newer import Fusion, Fun, Fun1
# from lib.models.layers.tbsi_layer import TBSILayer
# from lib.models.layers.New_Fusion import InterAct, Fusion
# from lib.models.layers.new_manner import Fusion, Fun
# from lib.models.layers.Enhance_SA_CA import Fusion, Fun
# from lib.models.layers.Newer import Fun1, Fun, Fusion
# from lib.models.layers.Newer_only_ACIIM import Fusion, Fun
# from lib.models.layers.EnhanceQ import Fusion, Fun
...
class VisionTransformerTBSI(BaseBackbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.tbsi_loc is not None and type(self.tbsi_loc) == list:
            for i in range(len(self.tbsi_loc)):
                self.Fusion_Layers.append(Fun1(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=self.tbsi_drop_path[i], norm_layer=norm_layer, act_layer=act_layer))
        self.Final_Fusion = Fusion(768, 12)
        self.init_weights(weight_init)

    def forward_features(self, z, x):
        B, H, W = x[0].shape[0], x[0].shape[2], x[0].shape[3]  #x.shape 24 3 256 256

        x_v = self.patch_embed(x[0])   # patch_embed:BCHW -> BNC   24 256 768
        z_v = self.patch_embed(z[0])   # 24 64 768
        ...
        z_i += self.pos_embed_z  # 24 64 768
        x_i += self.pos_embed_x  # 24 256 768

        if self.add_sep_seg:
            x += self.search_segment_pos_embed
            z += self.template_segment_pos_embed

        x_v = combine_tokens(z_v, x_v, mode=self.cat_mode)  # 24 (64+256) 768
        x_i = combine_tokens(z_i, x_i, mode=self.cat_mode)
        if self.add_cls_token:
            x = torch.cat([cls_tokens, x], dim=1)

        x_v = self.pos_drop(x_v) # 24 320 768
...
def vit_small_patch16_224_tbsi(pretrained=False, **kwargs):
    """
...
def vit_tiny_patch16_224_tbsi(pretrained=False, **kwargs):
    """
