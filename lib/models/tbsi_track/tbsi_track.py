...
from lib.models.tbsi_track.vit_tbsi_care import vit_base_patch16_224_tbsi, resize_pos_embed
...
class TBSITrack(nn.Module):
    ...
    def __init__(self, transformer, box_head, aux_loss=False, head_type="CORNER"):
        ...
        # self.Fusion_search = conv(hidden_dim * 2, hidden_dim)  # Fuse RGB and T search regions, random initialized
    ...
    def forward(self, template: torch.Tensor,
        ...
        out.update(aux_dict)
        out['backbone_feat'] = x
        out['x_v'] = aux_dict['x_v']
        out['x_i'] = aux_dict['x_i']
        return out
    ...
    def forward_head(self, cat_feature, gt_score_map=None):
        ...
        # opt_feat = self.Fusion_search(opt_feat)
        
        enc_opt = cat_feature[:, -num_search_token:, :]
        opt = (enc_opt.unsqueeze(-1)).permute((0, 3, 2, 1)).contiguous()
        bs, Nq, C, HW = opt.size()
        HW = int(HW/2)
        opt_feat = opt.view(-1, C, self.feat_sz_s, self.feat_sz_s)
