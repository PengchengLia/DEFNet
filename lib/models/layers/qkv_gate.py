...
class Difference_Module1(nn.Module):
    def __init__(self, dim=768, num_heads=12, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.scale = head_dim ** -0.5
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj1 = nn.Linear(dim//num_heads,dim//num_heads)
        self.proj2 = nn.Linear(dim//num_heads,dim//num_heads)
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x_vis, x_inf):
        ...
        attn_dif_vis = self.proj1(attn_dif_vis)
        attn_dif_vis = (attn_dif_vis.transpose(1, 2).reshape(B, N, C) * x_vis) + x_vis
        ...
        attn_dif_inf = self.proj2(attn_dif_inf)
        attn_dif_inf = (attn_dif_inf.transpose(1, 2).reshape(B, N, C) * x_inf) + x_inf
