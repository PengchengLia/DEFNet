...
class Difference_Module1(nn.Module):
    ...
    def forward(self, x_vis, x_inf):
        ...
        attn_vis = ((q_vis + q_inf) @ (k_vis.transpose(-2, -1))) * self.scale
        attn_vis = attn_vis.softmax(dim=-1)
        attn_vis = self.attn_drop(attn_vis)
        attn_vis = attn_vis @ v_vis
        attn_vis = self.dif_proj1(attn_vis)
        attn_vis = attn_vis.transpose(1, 2).reshape(B, N, C) + x_inf
        attn_vis = self.mlp(self.norm(attn_vis)) + attn_vis

        attn_inf = ((q_vis + q_inf) @ (k_inf.transpose(-2, -1))) * self.scale
        attn_inf = attn_inf.softmax(dim=-1)
        attn_inf = self.attn_drop(attn_inf)
        attn_inf = attn_inf @ v_inf
        attn_inf = self.dif_proj2(attn_inf)
        attn_inf = attn_inf.transpose(1, 2).reshape(B, N, C) + x_vis
        attn_inf = self.mlp(self.norm(attn_inf)) + attn_inf
        
        return attn_vis, attn_inf
