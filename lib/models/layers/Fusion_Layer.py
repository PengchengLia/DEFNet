...
class Difference_Module1(nn.Module):
    ...
    def forward(self, x_vis, x_inf):
        ...
        attn1 = (q_vis @ k_inf.transpose(-2, -1)) * self.scale
        attn1 = attn1.softmax(dim=-1)
        attn1 = self.attn_drop(attn1)
        attn1 = attn1 @ v_inf
        attn1 = self.dif_proj1(attn1)
        attn1 = attn1.transpose(1, 2).reshape(B, N, C) + x_vis

        attn2 = (q_inf @ k_vis.transpose(-2, -1)) * self.scale
        attn2 = attn2.softmax(dim=-1)
        attn2 = self.attn_drop(attn2)
        attn2 = attn2 @ v_vis
        attn2 = self.dif_proj2(attn2)
        attn2 = attn2.transpose(1, 2).reshape(B, N, C) + x_inf
